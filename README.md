...
Web 儀表板分為兩個部分：**管理介面**（需登入）和**客戶分享頁面**（公開）。

#### 管理介面（需登入 `/admin`）

管理介面可查看所有追蹤資料，包含尚未完成報告的工作項目：

| 路由 | 功能 |
|------|------|
| `/` | 工時總覽：所有客戶的本月/年度工時，可直接複製分享連結 |
| `/[clientSlug]` | 客戶年度報告：年度統計、月度趨勢圖、月份列表 |
| `/[clientSlug]/[period]` | 客戶月報：已完成報告 + 追蹤中的工作項目，可編輯計費工時 |
| `/admin/clients` | 客戶管理：新增、編輯客戶資訊 |
| `/admin/projects` | 專案管理：設定顯示名稱、關聯客戶、隱藏專案 |
| `/admin/contracts` | 合約管理：設定年度額度或月額度 |

#### 客戶分享頁面（公開，無需登入）

每個客戶自動產生一組分享連結 token，可將報告分享給客戶。分享頁面**只顯示已完成報告的工作項目**，不會顯示追蹤中的原始資料：

| 路由 | 功能 |
|------|------|
| `/share/[token]` | 客戶年度總覽：年度額度使用進度、月度趨勢圖 |
| `/share/[token]/[period]` | 客戶月報：已完成的工作項目清單、計費工時明細 |

**取得分享連結**：在首頁 `/` 的客戶列表中，點擊「複製分享連結」按鈕。
...
| 月額度 | 設定 `monthly_hours` | 每月額度（年度額度 = 12 × 月額度 + 結轉） |
